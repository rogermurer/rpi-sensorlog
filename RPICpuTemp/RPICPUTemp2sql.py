#!/usr/bin/env python3
""" This script reads the cpu temperature from a Rasperry PI and writes
the value to a postgresql database.
...
def read_temp(device_file):
    ...
    with open(device_file, 'r') as sensor_file:
        cpu_temp_raw = sensor_file.read()
    tempc = round(float(cpu_temp_raw) / 1000.0, 1)
    return tempc
...
def main():
    ...
    for sensor_id, device_file in sensors.items():
